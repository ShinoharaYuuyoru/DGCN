...
from drgcn_model_rnn import DRGCN
import drgcn_utils_joint_score_rnn

import matplotlib.pyplot as plt

class LinkPredict(nn.Module):
    def __init__(self, in_dim, h_dim, num_rels, num_bases=-1, num_rgcn_hidden_layers=1, use_self_loop=True,
                num_dkrl_hidden_layers=1, rnn_hidden_size=100, wordNum=0,
                dropout=0, reg_param=0):
        super(LinkPredict, self).__init__()

        # drgcn model
        self.drgcn = DRGCN(num_nodes=in_dim, h_dim=h_dim, out_dim=h_dim, num_rels=num_rels*2, num_bases=num_bases, num_rgcn_hidden_layers=num_rgcn_hidden_layers, use_self_loop=use_self_loop, 
                        num_dkrl_hidden_layers=num_dkrl_hidden_layers, rnn_hidden_size=rnn_hidden_size, wordNum=wordNum,
                        dropout=dropout)

        self.reg_param = reg_param
        self.w_relation = nn.Parameter(torch.Tensor(num_rels, h_dim))       # randomly init relation matrix
        # self.w_relation_inv = nn.Parameter(torch.Tensor(num_rels, h_dim))       # inverse relation. inspired by SimplE
    ...
    def get_loss(self, g, rgcnEmbedding, dkrlEmbedding,  triplets, labels):
        ...
        # cnn - cnn
        score1 = self.calc_score(dkrlEmbedding[triplets[:, 0]], r, dkrlEmbedding[triplets[:, 2]])
        # cnn - rgcn
        score2 = self.calc_score(dkrlEmbedding[triplets[:, 0]], r, rgcnEmbedding[triplets[:, 2]])
        # rgcn - cnn
        score3 = self.calc_score(rgcnEmbedding[triplets[:, 0]], r, dkrlEmbedding[triplets[:, 2]])
        # rgcn - rgcn
        score4 = self.calc_score(rgcnEmbedding[triplets[:, 0]], r, rgcnEmbedding[triplets[:, 2]])
        score = score1 + score2 + score3 + score4
        # score = torch.sigmoid(score)
        predict_loss = F.binary_cross_entropy_with_logits(score, labels)

        reg_loss1 = self.regularization_loss(dkrlEmbedding)
        reg_loss2 = self.regularization_loss(rgcnEmbedding)
...
def main(args):
    ...
    # create model
    model = LinkPredict(in_dim=num_nodes, h_dim=args.n_hidden, num_rels=num_rels, num_bases=args.n_bases, num_rgcn_hidden_layers=args.n_rgcn_layers, use_self_loop=True, 
                        num_dkrl_hidden_layers=args.n_dkrl_layers, rnn_hidden_size=args.n_rnn_hidden_size, wordNum=wordNum,
                        dropout=args.dropout, reg_param=args.regularization)
    ...
    test_graph, test_rel, test_norm = drgcn_utils_joint_score_rnn.build_test_graph(num_nodes, num_rels, train_data)
    ...
    adj_list, degrees = drgcn_utils_joint_score_rnn.get_adj_and_degrees(num_nodes, train_data)
    ...
    if os.path.exists(model_state_file):
        try:
            ...
            best_mrr = drgcn_utils_joint_score_rnn.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
    ...
    while True:
        ...
        g, node_id, edge_type, node_norm, data, labels = \
            drgcn_utils_joint_score_rnn.generate_sampled_graph_and_labels(
        ...
        loss = model.get_loss(g, rgcnEmbedding, dkrlEmbedding, data, labels)
        t1 = time.time()
        loss.backward()
        ...
        if epoch % args.evaluate_every == 0:
            ...
            with torch.no_grad():
                rgcnEmbedding, dkrlEmbedding = model(test_graph, test_node_id, test_rel, test_norm, allDescWordList)
            mixedEmbedding = getMixedEmbedding(args.embedding_mix_rate, rgcnEmbedding, dkrlEmbedding)
            mrr = drgcn_utils_joint_score_rnn.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
                                 valid_data, test_data, hits=[1, 3, 10], eval_p=args.eval_protocol)
            # save best model
            if mrr < best_mrr:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_rnn_hidden_size", type=int, default=100, help="Hidden size of RNN.")
