...
import drgcn_utils_joint_score
...
def main(args):
    ...
    test_graph, test_rel, test_norm = drgcn_utils_joint_score.build_test_graph(num_nodes, num_rels, train_data)
    ...
    adj_list, degrees = drgcn_utils_joint_score.get_adj_and_degrees(num_nodes, train_data)
    ...
    if os.path.exists(model_state_file):
        try:
            ...
            best_mrr = drgcn_utils_joint_score.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
    ...
    while True:
        ...
        g, node_id, edge_type, node_norm, data, labels = \
            drgcn_utils_joint_score.generate_sampled_graph_and_labels(
        ...
        if epoch % args.evaluate_every == 0:
            ...
            with torch.no_grad():
                rgcnEmbedding, dkrlEmbedding = model(test_graph, test_node_id, test_rel, test_norm, allDescWordList)
            mixedEmbedding = getMixedEmbedding(args.embedding_mix_rate, rgcnEmbedding, dkrlEmbedding)
            mrr = drgcn_utils_joint_score.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
                                 valid_data, test_data, hits=[1, 3, 10], eval_p=args.eval_protocol)
            # save best model
            if mrr < best_mrr:
