...
class LinkPredict(nn.Module):
    ...
    def getMixedEmbedding(self, rgcnEmbedding, dkrlEmbedding, node_id):
        ...
        mixRate = 0.8
        mix_embedding = rgcnEmbedding*mixRate + dkrlEmbedding*(1-mixRate)
    ...
    def get_loss(self, g, rgcnEmbedding, dkrlEmbedding, triplets, labels):
        ...
        # reg_loss = (torch.mean(mix_h.pow(2)) + torch.mean(mix_t.pow(2)) + torch.mean(r.pow(2))) / 3
        ...
        reg_loss = torch.mean(rgcnEmbedding.pow(2)) + torch.mean(self.w_relation.pow(2)) + torch.mean(dkrlEmbedding.pow(2))
...
def main(args):
    ...
    test_graph, test_rel, test_norm = drgcn_utils_rnn.build_test_graph(num_nodes, num_rels, train_data)
    ...
    adj_list, degrees = drgcn_utils_rnn.get_adj_and_degrees(num_nodes, train_data)

    # optimizer
    learningRate = args.lr
    optimizer = torch.optim.Adam(model.parameters(), lr=learningRate)

    model_state_file = 'drgcn_'+args.dataset+'_model_state_joint.pth'
    forward_time = []
    backward_time = []
    ...
    if os.path.exists(model_state_file):
        try:
            ...
            best_mrr = drgcn_utils_rnn.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
    ...
    while True:
        ...
        g, node_id, edge_type, node_norm, data, labels = \
            drgcn_utils_rnn.generate_sampled_graph_and_labels(
        ...
        if epoch % args.evaluate_every == 0:
            ...
            with torch.no_grad():
                rgcnEmbedding, dkrlEmbedding = model(test_graph, test_node_id, test_rel, test_norm, allDescWordList)
            mixedEmbedding = model.getMixedEmbedding(rgcnEmbedding, dkrlEmbedding, test_node_id)
            mrr = drgcn_utils_rnn.calc_mrr(mixedEmbedding, model.w_relation, torch.LongTensor(train_data),
                                 valid_data, test_data, hits=[1, 3, 10], eval_p=args.eval_protocol)
            # save best model
            if mrr < best_mrr:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--regularization", type=float, default=0.005, help="Regularization weight.")
    ...
    parser.add_argument("--n_rnn_hidden_size", type=int, default=20, help="Hidden size of RNN.")
